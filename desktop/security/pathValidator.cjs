/**
 * DM ARRANGIA - Desktop Security & Path Validation Module
 * Enforces strict capability-based filesystem boundaries, path traversal rejection,
 * token validation, safe external URL filtering, and IPC sender validation.
 */
...
const TOKEN_REGEX = /^(dir|file)_[a-zA-Z0-9_-]{8,64}$/;

/**
 * Creates a safe structured error object that never leaks local system paths.
 */
function createSafeError(code, rawMessage) {
  // Sanitize any accidental absolute paths in the error message
...
/**
 * Validates whether an external URL uses an approved secure protocol.
 */
function isSafeExternalUrl(rawUrl) {
  ...
  // Reject dangerous pseudo-protocols upfront
...
/**
 * Strict path validation to ensure a relative path NEVER escapes the approved root directory.
 */
function validateRelativePath(approvedRoot, relativePath) {
  ...
  // Check for null bytes (both raw and URL-encoded)
  ...
  // Reject absolute paths across POSIX, Windows, and UNC formats
  if (
  ...
  // Reject explicit .. path traversal in raw segments
  ...
  // Test decoded URL sequences for encoded traversals (e.g., %2e%2e or %2f)
  try {
  ...
  // Canonical path resolution & containment check
  const canonicalRoot = path.resolve(approvedRoot);
  const resolvedTarget = path.resolve(canonicalRoot, relativePath);

  // The resolved path MUST begin with the approved root + path separator
  ...
  // If the target exists on disk, check for symlink escapes
...
/**
 * Validates an opaque directory token against the main process registered store.
 */
function validateDirectoryToken(token, directoryStore) {
  ...
  return { valid: true, directory: entry };
}

/**
 * Validates an opaque file token against the main process registered store.
 */
function validateFileToken(token, fileStore) {
  if (typeof token !== 'string' || !TOKEN_REGEX.test(token)) {
  ...
  return { valid: true, file: entry };
}

/**
 * Validates that an IPC invocation originates strictly from the authorized main window renderer.
 */
function validateIpcSender(event, mainWindow, isDev, devUrl) {
  ...
  // Ensure sender WebContents matches the main window
  if (event.sender !== mainWindow.webContents) {
    return { authorized: false, reason: 'IPC call originates from unauthorized WebContents.' };
  }

  // Ensure call comes from the main frame, not an untrusted child iframe
  ...
  if (isDev) {
    ...
    // In production, must be loaded from local file:// inside app
