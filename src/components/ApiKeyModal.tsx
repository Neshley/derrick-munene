      ...
      >
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-zinc-800/80 bg-zinc-900/60">
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <div>
              <h2 className="text-sm sm:text-base font-bold font-mono text-zinc-100 flex items-center gap-2">
          ...
          <button
            ...
            onClick={onClose}
            className="p-1.5 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/60 rounded-lg transition-colors cursor-pointer shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 space-y-3.5 sm:space-y-4">
          {/* Security Guarantee */}
          <div className="p-3 sm:p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/30 flex items-start gap-2.5 sm:gap-3">
            <Lock className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400 shrink-0 mt-0.5" />
            <div className="text-xs text-zinc-300 space-y-1">
              <p className="font-semibold text-emerald-300 text-xs sm:text-sm">Server-Protected Credentials</p>
          ...
          {/* Current Status Card */}
          <div className="p-3.5 sm:p-4 rounded-xl bg-zinc-900/80 border border-zinc-800 space-y-2.5 sm:space-y-3">
            ...
            {serverInfo?.message && (
              <div className="pt-2 border-t border-zinc-800 text-[10px] sm:text-[11px] text-zinc-400 font-mono">
                {serverInfo.message}
              </div>
            )}
          </div>

          <div className="text-[11px] sm:text-xs text-zinc-400 leading-relaxed">
        ...
        {/* Footer Actions */}
        <div className="flex flex-col-reverse xs:flex-row items-stretch xs:items-center justify-between gap-2 px-4 sm:px-6 py-3 sm:py-4 border-t border-zinc-800/80 bg-zinc-900/60">
          <button
            id="btn-recheck-api-status"
            type="button"
            onClick={fetchStatus}
            disabled={status === 'checking'}
            className="w-full xs:w-auto justify-center px-4 py-2 rounded-xl text-xs font-mono font-bold text-zinc-200 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 disabled:opacity-50 flex items-center gap-2 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            type="button"
            onClick={onClose}
            className="w-full xs:w-auto justify-center px-5 py-2 rounded-xl text-xs font-mono font-bold text-zinc-950 bg-amber-400 hover:bg-amber-300 shadow-md flex items-center gap-1.5 transition-all cursor-pointer"
          >
